...
const TOOLS = [
  ...
  },
  {
    ...
    "icon": "📷",
    "updated": "2026-08-27",
    "help": "Paste a CompanyCam access token (from app.companycam.com/access_tokens — needs a Pro/Premium/Elite plan) and it pulls the ENTIRE account: every project, then one document call per project, relayed through this site's own /api/companycam function because CompanyCam sends no CORS headers. CompanyCam silently caps per_page below what you ask for, so the tool learns the server's real page size instead of assuming it — the reason an earlier build stopped at 50 projects. Fetching is paced under CompanyCam's documented 240 GET/minute budget with 429 backoff, shows a progress bar with a running ETA, and can be stopped at any point keeping whatever loaded. The preview lists projects with totals (projects, documents, with/without documents, total size), is searchable by project name, address or document name, and pages at 100/250/500/1000 rows; document names render only for rows you expand. Then connect a Zuper account with an API key (region auto-detected) and fetch its properties and customers. Each project is auto-matched to a property on project name, project address and property address — noise words like LLC/Roofing/Construction are stripped and an exact street + zip match scores 100% — via a token index so a large account maps in seconds rather than comparing every project to every property. The mapping table pages too, filters by will-sync/needs-review/skipped, and its property picker loads its options only when focused. Documents are attached to the customer linked to the matched property via POST customers/{customer_uid}/attachments; Zuper has no property-level attachment endpoint, so that is the real target, and a property with no linked customer is reported and skipped. Zuper stores the CompanyCam document URL rather than a copy of the file, so nothing is downloaded or re-uploaded — the tool flags any URL carrying a signature or expiry, because those attachments would stop resolving later. Dry run is on by default; projects and mapping and results all export to CSV. Re-runs are deduped only against a record kept in the local browser, since Zuper exposes no way to list a customer's existing attachments."
  }
];
